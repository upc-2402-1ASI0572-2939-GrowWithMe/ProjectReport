...
| TS01            | Crear API para guardar objetivos            | Como usuario, quiero enviar datos de objetivos al backend para que se almacenen correctamente en la base de datos.                                                         | **Scenario 1:** Given que el usuario envía datos válidos a través del endpoint `/api/objetivos`, when el backend recibe la solicitud, then se almacenan correctamente los datos del objetivo. <br><br> **Scenario 2:** Given que el usuario envía datos incompletos o inválidos, when el backend recibe la solicitud, then se devuelve un error 400 con el mensaje "Datos incompletos o inválidos". | No corresponde             |
| TS01            | Implementar lógica combinación datos objetivos | Como usuario, quiero que los datos manuales y de sensores se combinen correctamente para tener una visión completa de los objetivos.                                         | **Scenario 1:** Given que los datos manuales y de sensores están disponibles, when la lógica combina los datos, then los datos combinados se almacenan correctamente y son accesibles para visualización. <br><br> **Scenario 2:** Given que uno de los conjuntos de datos está incompleto, when se intenta combinar los datos, then se devuelve un error 400 con el mensaje "Faltan datos necesarios para la combinación". | No corresponde             |
| TS01            | Crear API para consultar objetivos          | Como usuario, quiero consultar los objetivos almacenados para poder visualizarlos en la aplicación.                                                                        | **Scenario 1:** Given que el usuario realiza una solicitud GET a `/api/objetivos`, when el backend devuelve los datos de los objetivos almacenados, then el usuario puede visualizar la lista de objetivos correctamente. <br><br> **Scenario 2:** Given que no existen objetivos almacenados, when el usuario consulta los objetivos, then se devuelve un mensaje vacío con código 200 y el mensaje "No se han encontrado objetivos". | No corresponde             |
| TS01            | Implementar lógica seguimiento objetivos    | Como usuario, quiero ver el seguimiento del progreso de mis objetivos usando los datos actuales para tomar decisiones informadas.                                            | **Scenario 1:** Given que existen datos de progreso almacenados para los objetivos, when el usuario consulta el progreso, then se muestran los avances correctamente. <br><br> **Scenario 2:** Given que no hay datos de progreso disponibles, when el usuario consulta el progreso, then se muestra un mensaje de error con código 404: "No se han encontrado datos de progreso". | No corresponde             |
| TS02            | Crear API para guardar estrategia cultivo   | Como usuario, quiero guardar estrategias de cultivo para recibir recomendaciones basadas en datos.                                                                          | **Scenario 1:** Given que el usuario envía datos válidos de la estrategia de cultivo a través del endpoint `/api/estrategias`, when el backend recibe la solicitud, then la estrategia se guarda correctamente. <br><br> **Scenario 2:** Given que el usuario envía datos incompletos o inválidos, when el backend recibe la solicitud, then se devuelve un error 400 con el mensaje "Datos incompletos o inválidos para la estrategia". | No corresponde             |
| TS02            | Implementar lógica validación estrategia    | Como usuario, quiero que se valide la estrategia de cultivo con datos actuales para asegurar su viabilidad.                                                                  | **Scenario 1:** Given que el usuario envía una estrategia válida, when la estrategia es validada, then el sistema confirma que la estrategia es viable. <br> **Scenario 2:** Given que el usuario envía una estrategia inválida, when se valida la estrategia, then se devuelve un error 400 con el mensaje "Estrategia no válida". | No corresponde             |
| TS02            | Crear API para consultar estrategias        | Como usuario, quiero consultar estrategias de cultivo almacenadas para analizar su contenido.                                                                               | **Scenario 1:** Given que el usuario realiza una solicitud GET a `/api/estrategias`, when el backend devuelve las estrategias almacenadas, then el usuario puede visualizar la lista de estrategias correctamente. <br><br> **Scenario 2:** Given que no existen estrategias almacenadas, when el usuario consulta las estrategias, then se devuelve un mensaje vacío con código 200 y el mensaje "No se han encontrado estrategias". | No corresponde             |
| TS02            | Implementar lógica seguimiento impacto estrategia | Como usuario, quiero ver el impacto de mis estrategias de cultivo para optimizar la productividad.                                                                           | **Scenario 1:** Given que existen datos sobre el impacto de las estrategias de cultivo, when el usuario consulta los resultados del impacto, then se muestra el impacto de las estrategias correctamente. <br><br> **Scenario 2:** Given que no existen datos de impacto, cuando el usuario consulta el impacto de las estrategias, entonces se muestra un mensaje de error con código 404: "No se han encontrado datos de impacto". | No corresponde             |
| TS03-T1         | Crear API para registrar actividades        | Como usuario, quiero registrar actividades agrícolas para llevar un historial detallado de mis acciones.                                                                  | **Scenario 1:** Given que el usuario envía datos válidos de la actividad agrícola a través del endpoint `/api/actividades`, when el backend recibe la solicitud, then la actividad se registra correctamente. <br><br> **Scenario 2:** Given que el usuario envía datos incompletos o inválidos, when el backend recibe la solicitud, then se devuelve un error 400 con el mensaje "Datos incompletos o inválidos para la actividad". | No corresponde             |
| TS03            | Implementar lógica integración datos actividad | Como usuario, quiero combinar datos manuales y automáticos de mis actividades para tener un control más completo.                                                           | **Scenario 1:** Given que el usuario tiene datos manuales y automáticos de la actividad, when la lógica de integración procesa los datos, then los datos combinados se almacenan correctamente. <br><br> **Scenario 2:** Given que faltan datos manuales o automáticos, when la lógica de integración intenta combinar los datos, then se devuelve un error 400 con el mensaje "Faltan datos para la integración". | No corresponde             |
| TS03            | Crear API para consultar actividades        | Como usuario, quiero consultar mis actividades registradas para poder analizarlas y tomar decisiones.                                                                      | **Scenario 1:** Given que el usuario realiza una solicitud GET a `/api/actividades`, when el backend devuelve los datos de las actividades registradas, then el usuario puede visualizar la lista de actividades correctamente. <br><br> **Scenario 2:** Given que no existen actividades registradas, when el usuario consulta las actividades, then se devuelve un mensaje vacío con código 200 y el mensaje "No se han encontrado actividades". | No corresponde             |
| TS04            | Crear API para registrar datos calidad      | Como usuario, quiero registrar los datos de calidad con apoyo de sensores para monitorear el estado de mis cultivos.                                                         | **Scenario 1:** Given que el usuario envía datos válidos de calidad a través del endpoint `/api/calidad`, when el backend recibe la solicitud, then los datos de calidad se registran correctamente. <br><br> **Scenario 2:** Given que el usuario envía datos incompletos o inválidos, when el backend recibe la solicitud, then se devuelve un error 400 con el mensaje "Datos incompletos o inválidos para la calidad". | No corresponde             |
